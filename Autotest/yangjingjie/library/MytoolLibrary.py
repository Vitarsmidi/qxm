...
#db_user = 'sims_js2200'
#db_paswd = 'sims_js2200_10g'
#db_dsn = '10.1.3.234:1521/ora9i'
db_user = 'sims_bj2200'
db_paswd = 'sims_bj2200_10g'
db_dsn = '10.1.4.59:1521/ora10g'
#db_user = 'sims_cs2200'
#db_paswd = 'sims_cs2200_10g'
...
def execute_procedure_func(proc_name, paralist):
    '''
    '''
    proc_list = []
    if paralist.find("|") == -1:
        proc_list.append(paralist)
    else:
        proc_list = paralist.split("|")
        
    db = cx_Oracle.connect(db_user, db_paswd, db_dsn)
    cursor_fetch = db.cursor()
    result = cursor_fetch.callproc(proc_name,proc_list)
    cursor_fetch.close()
    db.commit()
    return result
...
def parse_message_xmlstring_l(xmlstring1):
    '''
    '''
    builtin = BuiltIn().get_library_instance('BuiltIn')
    fp = open(r'./message_tmp2.xml', "wb")
    fp.write(xmlstring1.decode('GBK').encode('utf-8'))
    fp.close()
    
    parser = etree.XMLParser(strip_cdata=False)
    xmls1 = etree.parse(r'./message_tmp2.xml', parser)
    
    builtin.set_test_variable('${XML_DATA_l}', xmls1)
...
def get_value_from_xmls_by_node_l(message_node_l):
    '''
    
    '''
    builtin = BuiltIn().get_library_instance('BuiltIn')
    xmls1 = builtin.get_variable_value('${XML_DATA_l}')

    for xms1 in xmls1.getiterator():
        if message_node_l == xms1.tag:
            messge_data_l = xms1.text 
    return messge_data_l
...
def check_message_with_database_charge(messge_data,db_data,str_format):
    '''
    
    '''
    messge_data = str(messge_data)
    db_data = str(db_data)
    messge_data = filter(str.isdigit,messge_data)
    #db_data = filter(str.isdigit,db_data)
    db_data = int(db_data)
    messge_data = int(messge_data)
    messge_data = messge_data*100
...
def get_text(nodes):
    return [unicode(node.xpath('text()')) for node in nodes]

def get_value_from_xmls_by_xpath_A(node_xpath):
    '''
    ...
    xmls = builtin.get_variable_value('${XML_DATA}')
   # if len(find_key) == 1:
   # return xmls.xpath('//Icon/IconId/text()')[0]
    node = xmls.xpath(node_xpath)
    s = get_text(node)
    db_data = str(db_data)
    s = str(s)
    a = filter(str.isdigit,s )
    return a
